        ...
        >
          ...
          <div class="dataset-stats">
            ...
            <div class="stat-row">
              ...
              <span class="stat-value">{{ dataset.companies ? dataset.companies.length : (dataset.companyCount || dataset.totalRows || 0) }}</span>
            </div>
            ...
            <div v-if="dataset.scrapingStatus === 'in_progress' || dataset.scrapingStatus === 'pending'" class="stat-row">
              ...
              <span class="stat-value">{{ dataset.scrapingStats?.processedDomains || dataset.scrapingStats?.processedUrls || 0 }} / {{ dataset.companies ? dataset.companies.length : (dataset.companyCount || dataset.totalRows || 0) }}</span>
            </div>
  ...
  methods: {
    ...
    getDatasetStatusText(dataset) {
      if (this.isValidationDataset(dataset)) {
        const validation = this.getValidationField(dataset)
        const status = validation?.status || 'not_started'
        return this.getValidationStatusText(status)
      }
    ...
    getDatasetStatusClass(dataset) {
      if (this.isValidationDataset(dataset)) {
        const validation = this.getValidationField(dataset)
        const status = validation?.status || 'not_started'
        return this.getValidationStatusClass(status)
      }
