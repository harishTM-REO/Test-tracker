...
export default {
  ...
  methods: {
    ...
    getAllWebsitesData() {
      this.disableBtn = true;
      axios.get('http://localhost:3000/getWebsites', {
        url: this.websiteURL,
      })
        .then((response) => {
          this.responseData = response.data;
...
<template>
  <v-app class="main-background">
    ...
    <v-app-bar app color="white" elevation="1" v-if="!showloader" class="app-bar">
      ...
      <v-btn text class="nav-btn" @click="$router.push('/monitored-websites')">Monitored Sites</v-btn>
      <v-btn text class="nav-btn" @click="$router.push('/add-website')">Add Website</v-btn>
    </v-app-bar>
