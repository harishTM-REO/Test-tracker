...
<template>
  <header class="text-center align-center justify-center">
    REO - CRO TOOL DETECTOR
  </header>
  <main>
    <v-row class="mt-6 align-center justify-center">
      <v-col cols="6" dark>
        <v-form @submit.prevent="handleSubmit">
          <v-text-field dark label="Website to check" v-model="websiteURL" hide-details="auto" clearable></v-text-field>
        </v-form>
      </v-col>
      <v-col cols="2" class="mb-6">
        <v-btn class="text-subtitle-1 mt-6 secondary-text" dark @click="handleClick(event)" :disabled="disableBtn">
          Check</v-btn>
      </v-col>
    </v-row>
    <v-row class="mt-6 align-center justify-center">
    ...
    <v-row v-if="responseData.length">
      <h3 class="text-h3 text-white mt-6 mb-6 pl-3 ternary-text">{{ responseData[0].site.split(".")[1].toUpperCase()
        }}
      </h3>
      <!-- <v-col cols="12" dark>
        <v-table>
              ...
              <v-table>
                ...
                <tbody>
                  <tr v-for="(experimentState, ind) in item.experimentStates" :key="ind">
                    ...
                    <td class="text-center primary-text">{{ `${experimentState.isActive ? "Inactive" : "Active"}` }}
                    </td>
                  </tr>
                </tbody>
              </v-table>
