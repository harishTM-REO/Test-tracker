...
class AdobeTargetValidationJobService {
  static getWorkerUrl() {
  ...
  static async startValidation(datasetId) {
    try {
      console.log(`🚦 Starting Adobe Target validation for dataset ${datasetId}`);
      ...
      const workerUrl = `${this.getWorkerUrl()}/at10/api/validation`;
      ...
      console.error('❌ Failed to start Adobe Target validation:', error.message);
      ...
      try {
        ...
        if (dataset) {
          dataset.adobeTargetValidation = {
            ...
            status: 'failed',
            summary: {
              totalUrls: dataset.adobeTargetValidation?.summary?.totalUrls || 0,
              positiveCount: 0,
              negativeCount: 0,
              failedCount: 0,
              detectionRate: 0
...
module.exports = AdobeTargetValidationJobService;
