...
class DynamicYieldValidationJobService {
  static getWorkerUrl() {
  ...
  static async startValidation(datasetId) {
    try {
      console.log(`🚦 Starting Dynamic Yield validation for dataset ${datasetId}`);
      ...
      dataset.dynamicYieldValidation = {
        ...(dataset.dynamicYieldValidation || {}),
        status: 'pending',
        lastRunAt: new Date(),
        summary: {
      ...
      const workerUrl = `${this.getWorkerUrl()}/at10/api/dynamic-yield-validation`;
      console.log(`🔗 Triggering validation worker: ${workerUrl}`);
      ...
      console.error('❌ Failed to start Dynamic Yield validation:', error.message);
      ...
      try {
        ...
        if (dataset) {
          dataset.dynamicYieldValidation = {
            ...
            status: 'failed',
            summary: {
              totalUrls: dataset.dynamicYieldValidation?.summary?.totalUrls || 0,
              positiveCount: 0,
              negativeCount: 0,
              failedCount: 0,
              detectionRate: 0
...
module.exports = DynamicYieldValidationJobService;
